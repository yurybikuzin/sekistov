...
macro_rules! pasitos {
    ...
    // delay
    (
        delay $case:ident { $($item:tt)* }
        cancellable_by $cancellation_token:ident
        for $delay_duration:expr
    ) => {
    ...
    (@push
        ...
        paste::paste! {
            ...
            if in_progress < max_at_once {
                ...
                for i in 0..capacity {
                    if let Some(item) = pasitos!($mod pop_front) {
                        $mod::inc_in_progress();
